...
export default function ProductsScreen() {
  ...
  const handleOpenEdit = (product: Product) => {
    ...
    setFormProtein(
      product.proteinPerServing !== null
        ? String(product.proteinPerServing)
        : "",
    );
    setFormFat(
      product.fatPerServing !== null ? String(product.fatPerServing) : "",
    );
    setFormCarbs(
      product.carbsPerServing !== null ? String(product.carbsPerServing) : "",
    );
    setModalVisible(true);
  };
  ...
  const handleSave = async () => {
    ...
    const cal = Math.round(calculateValue(formCalories));
    if (cal < 0) {
      Alert.alert(
        "入力エラー",
        "カロリーに正しい数値または計算式を入力してください。",
      );
      return;
    }
              ...
              <View style={styles.productInfo}>
                ...
                <Text style={[styles.productPFC, { color: colors.icon }]}>
                  ...
                  {product.proteinPerServing !== null
                    ...
                    : "-"}{" "}
                  F:
                  {product.fatPerServing !== null
                    ? `${product.fatPerServing}g`
                    : "-"}{" "}
                  C:
